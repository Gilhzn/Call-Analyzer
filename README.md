...
- **התוצאות חיות רק בדפדפן.** כדי לאפשר המשך אחרי ניתוק או רענון (למשל דפדפן בטלפון שעבר לרקע), אירועי העיבוד נשמרים **בזיכרון בלבד** למשך עד `JOB_TTL_SECONDS` (ברירת מחדל 10 דקות) ואז נמחקים אוטומטית.

## 🌐 גרסת הדפדפן — לינק ציבורי (GitHub Pages)

**<https://gilhzn.github.io/Call-Analyzer/>** — נפתח מכל מקום, ללא התקנה.

בגרסת הדפדפן הכול רץ **בתוך הדפדפן שלכם** (תמלול עם transformers.js — מואץ WebGPU כשזמין — וניתוח עם WebLLM שנטען במקביל לתמלול) — הקובץ לא נשלח לשום שרת, אפילו לא לשרת של האפליקציה. בהפעלה הראשונה יורדים קובצי מודל (חד-פעמי, נשמר במטמון). הניתוח העסקי דורש דפדפן עם WebGPU (Chrome/Edge עדכניים במחשב); ללא WebGPU יופק תמליל בלבד. איכות ומהירות מרביות — בגרסה המקומית או ב-Codespaces שלמטה.
...
## ⚡ האצת ענן חינם — GPU של Groq

בשתי הגרסאות אפשר להריץ את התמלול והניתוח על שרתי ה-GPU/LPU של [Groq](https://groq.com) — **בחינם, ללא כרטיס אשראי** — עם `whisper-large-v3-turbo` (התמלול המדויק ביותר, שיחה של חצי שעה בעשרות שניות) ומודל `gpt-oss-120b` לניתוח (קפיצת איכות עצומה לעומת מודלים מקומיים קטנים):

1. מנפיקים מפתח חינם ב-<https://console.groq.com/keys>.
2. **בגרסת הדפדפן:** מסמנים "האצת ענן חינם (GPU)" ומדביקים את המפתח — הוא נשמר בדפדפן בלבד. עובד גם בטלפון ובדפדפנים ללא WebGPU.
3. **בגרסה המקומית:** מגדירים `GROQ_API_KEY` ב-`.env`.

מגבלות החינם נדיבות לשימוש אישי: ~2 שעות אודיו לשעה ו-2,000 תמלולים ביום; ניתוח — 30 בקשות לדקה. אם ההאצה נכשלת מכל סיבה, האפליקציה חוזרת אוטומטית לעיבוד מקומי.

> שקלול פרטיות: במצב האצה ההקלטה נשלחת ל-Groq לעיבוד (כפוף למדיניות הפרטיות שלהם) ואינה נשמרת על ידי האפליקציה. לפרטיות מוחלטת השאירו את המצב כבוי — הכול יעובד מקומית.
